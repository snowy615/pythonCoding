...
def tryPath(x, y):
    ...
    for i in range(8):
        ...
        if 0 <= a <= 4 and 0 <= b <= 7 and used[b][a] == 0:
            if mgrid[y][x] == mgrid[b][a] or len(path) > 1 and 2 * mgrid[y][x] == mgrid[b][a]:
                notMove = False
                used[b][a] = 1
                path.append([a, b])
                tryPath(a, b)
...
def play2248(boardValues):
    global grid, maxPath, path, mgrid, tmp
...
#play2248("4096 2048 1024  512  256 4096  128   64   32 4096 32 2048 1024  512  256  512  128   64   32 4096  32 2048 1024  256  512  256  256  256  256   64   32  128   64  256 1024 1024  128  256   64   32")